...
df = pd.read_csv("/Users/samuel/Desktop/temp.csv", sep=" ",header=None, names=['berth', 'platoon', 'line', 'tt_mean', 'bus_flow', 'pax_flow', 'bus_arrival_cv', 'tt_cv', 'mode', 'stop', 'delay', 'service'])
df.head()
nrow = df['bus_flow'].nunique() * df['pax_flow'].nunique() * df['tt_cv'].nunique()
ncolumn = df['bus_arrival_cv'].nunique()*2
# fig, axes = plt.subplots(nrows=nrow, ncols=ncolumn, figsize=(20, 45), gridspec_kw = {'width_ratios':[1, 1, 1]})
fig, axes = plt.subplots(nrows=nrow, ncols=ncolumn, figsize=(35, 45*2))
axes = axes.ravel()
lineStyle = ["-", "-.", "--", ":", "-."]
